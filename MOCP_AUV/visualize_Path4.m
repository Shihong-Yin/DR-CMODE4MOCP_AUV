function [Fitness, Constraint] = visualize_Path4(problemNum, solution, algorithm, problem)
...
for m = 1:size(solution, 1)
    ...
    for k = 1:AUV_num
        ...
        if ENV.currTemporal.noise_std > 0
            waterFlow_x = waterFlow_x + ENV.currTemporal.noise_std * randn(size(waterFlow_x));
            waterFlow_y = waterFlow_y + ENV.currTemporal.noise_std * randn(size(waterFlow_y));
            waterFlow_z = waterFlow_z + ENV.currTemporal.noise_std * randn(size(waterFlow_z));
        end
